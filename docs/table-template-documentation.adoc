...
When driving overlays from your own Red code, `do-code-overlay` takes `face`, `data-x`, and `data-y` only (a former `cell` argument was removed).
...
* `data-table-link` - Opens a link dialog to pick or edit a related value; writes go through `set-data/index/refresh` so row/column indices stay aligned with stored addresses. The `vid-lib` entry uses `datatype: "integer"`. Optional `/pre-built` setup supplies sample column text. Data columns only.
