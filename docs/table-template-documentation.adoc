...
* When Sheet mode is enabled, you can NOT define the table data file using the `data` option.
...
When driving overlays from your own Red code, `do-code-overlay` takes `face`, `data-x`, and `data-y` only (a former `cell` argument was removed). Code and VID overlay execution exits immediately when `is-filtered? face` is true, so overlays do not run on filtered views.
...
**Note:** Prefer `get-cells` for reading arbitrary cells by column and row in overlays. `get-table-data` remains available as a thin wrapper over the table `get-data` actor (including `/row`), for compatibility with older examples.
**Note:** Prefer `set-cells` for writing data cells from overlays. `set-table-data` is retained for compatibility but is deprecated in favor of `set-cells`.

==== Overview of functions
image::images/table-user-function-v2.png[]
...
DESCRIPTION: 
     Returns the table data at the col and row given. ONLY available in the Code and VID Overlay context. Operates ONLY on Data cells. Implemented as a thin wrapper over `table-obj/actors/get-data` (same refinements as `get-cells` where applicable).
...
DESCRIPTION: 
     Returns TRUE if the current row is NOT a first or last data row. Only available and useful in the Code and VID Overlay context. 
     MID-ROW? is a function! value.
----
...
DESCRIPTION: 
     When called on the last data row, sums the current column across middle data rows (all row-index entries except first and last), writes the total to the current cell, and sets cell color to yellow. Only available in the Code and VID Overlay context alongside first-row?, last-row?, and mid-row?.
     TOTAL-THIS-COL is a function! value.
----
...
REFINEMENTS:
     /refresh     => refresh the view after the cell is modified.
----

[source,red]
----
USAGE:
     SET-CELLS col-id row-id val

DESCRIPTION: 
     Modify the table data. ONLY available in the Code and VID Overlay context. Operates ONLY on Data cells. 
     SET-CELLS is a function! value.
...
DESCRIPTION: 
     Modify the table data. ONLY available in the Code and VID Overlay context. Operates ONLY on Data cells. *Deprecated:* prefer `set-cells` for new overlay code; signature and refinements match `set-cells`.
...
DESCRIPTION: 
     Appends a row of data at the end of existing data. Blocked when the table is filtered (`status-msg/alert`).
...
DESCRIPTION: 
     Append a column to the existing table data. Blocked when the table is filtered. Returns `none` when `/ret-col-num` is not used.
...
----
USAGE:
     TABLE1/ACTORS/APPEND-VIRTUAL-COL face

DESCRIPTION: 
     Append a virtual column. Blocked when the table is filtered.
     TABLE1/ACTORS/APPEND-VIRTUAL-COL is a function! value.

REFINEMENTS:
     /name header-name => sets virtual column source, code, and data header fields from the supplied string.
----
...
DESCRIPTION: 
     sets table data and refreshes table display. By default uses visible/drawn row and column NOT data index numbers. Calls `trigger-on-changed face` after modifying data.
     <table-name>/ACTORS/SET-DATA is a function! value.

ARGUMENTS:
     face         [object!] 
     col-id       [integer! string!] {A col-id of string! will attempt to match a currently configured column name.}
     row-id       [integer!] {Visual (drawn) row number by default; use `/index` for data row indices.}
     val          [any-type!] 
...
=== Helper Functions
==== Displaying Table Data and Configuration
[source,red]
----
USAGE:
     PRINT-TABLE-DATA face

DESCRIPTION: 
     Prints data rows (skipping the header row) via `print-table/columns/column-names/name/col-sequence`, honoring current `col-index` (virtual columns removed), table `name`, and header row.
     PRINT-TABLE-DATA is a function! value.
----

[source,red]
----
Example Usage:
table1/actors/print-table-data table1
----

[source,red]
----
USAGE:
     PRINT-TABLE-CONFIG face

DESCRIPTION: 
     Displays table configuration details (external helper, same pattern as the former table-details action).
     PRINT-TABLE-CONFIG is a function! value.
----
...
The table style supports several mechanisms for detecting when data or layout has changed.

==== Cell edit notification (`on-change`)

The `face/change-state` can be `'before` or `'after`, and `face/active` contains the address of the changed cell. The address returned is a pair of the column number and the row number.
...
==== Change flag and hooks (`changed?`, `on-changed`, `on-saved`)

* `face/changed?` — boolean flag (`#(false)` by default) used by the change-notification pipeline. Reset to `#(false)` by many view and layout operations (refresh, hide/unhide, filter, unfilter, sort, unsort, resize, and related paths). Set by `trigger-on-changed` on the first change after a reset.
* `on-changed` — empty overridable hook on the table face; invoked when the table is considered changed (after the first `trigger-on-changed` call while `changed?` is already true).
* `trigger-on-changed` — actor that sets `face/changed?` on first call after a reset; on subsequent calls while already changed, invokes `on-changed`. Called from `set-data`, `set-column-names`, and `fill` (after `auto-save`).
* `on-saved` — empty overridable hook; invoked from `save-table` after a successful save.

Example overriding hooks in a layout:

[source,red]
----
view [
    table1: table data %mydata.redtbl with [
        on-changed: func [face][ print ["table changed:" face/name] ]
        on-saved: func [face][ print ["table saved:" face/name] ]
    ]
]
----

==== Table name (`face/name`)

When a table is opened from a file, `face/name` is set from the filename (without extension). When `face/data` is not a file, `face/name` is `"none"`. The name is used by `print-table-data` and related diagnostics.
...
NOTE:  `set-table-data` is deprecated in overlay code; prefer `set-cells` for new work.
...
NOTE: Moving a row up or down is blocked when either the source or target row index is in `frozen-rows`. Unfreeze the affected rows first; the status line reports when a move is blocked.
...
Sets static names for columns (requires table to be loaded from a file, not just created from a block). These names are used to identify the columns in the table and are not affected by sorting, filtering or what headings are defined in the table data. These names are used by the `get-cell`, `set-cell`, `get-cells`, `set-cells`, `get-table-data`, and `set-table-data` functions to get and set the definitive value of a cell. This allows the table to be used like a database with named columns.
...
* **Information**
  ** **Print Table Data** - Print data rows to the console (skipping header row)
  ** **Table Configuration** - Show table configuration details
  ** **Print Code Overlays** - Print overlay source to the console for debugging
* **Refresh View** - Rebuild the display from current data and overlays
...
* **Paste** - Paste cells
* **Color**
  ** **Set Color** - Set color for selected cells (opens `request-color` when no color is passed)
  ** **Remove Color** - Clear per-cell color entries from `face/colors` for the current selection
...
When a column type is set, all data in that column is converted to the specified type. Any cells containing a data type that doesn't match are cleared out. If a row is frozen, it will remain unmodified during that conversion.

When datatype conversion runs during VID-driven cell editing, cells on frozen rows skip coercion and keep the raw edit text (`tx`), so header-like frozen rows are not forced into the column type.
...
* `data-request-date` - Date column with a small control that opens `request-date` and writes the chosen value back through `set-data/index/refresh`. Data columns only. See <<_data_request_date>>.
* `data-table-link` - Opens a link dialog to pick a related value; writes go through `set-data/index/refresh` so row/column indices stay aligned with stored addresses. The `vid-lib` entry uses `datatype: "integer"`. Optional `/pre-built` setup supplies sample column text. Data columns only. Link cell label format is `link-data (#id)`. When running inside table-stack, a context menu offers *Goto Linked Table* (uses `table-stack-tab-panel` and `find-tab-index`) and *Goto Linked Row*; outside table-stack, *Goto Linked Table* shows a status message.
...
Repeating VID overlay for `date!` columns. Each cell shows the current date value and a control that runs `request-date`; the result is applied with `set-data/index/refresh` so data coordinates stay consistent after scroll or sort. Styling follows the same pattern as other repeating `data-` overlays (`vid-draw`, `vid-lib` with `datatype: "date"`, `overlay-config`, `setup-data-request-date`). F4 restore rebinds overlay layout like other repeating VID types.
...
The multi-line text editor (also opened from the cell menu) uses a default dialog size of 900×300 pixels.
...
`actors/set-data` uses visual (drawn) row and column positions by default. Use `/index` when `col-id` and `row-id` are data indices (string column names are still resolved to the correct data column).
...
* `set-vid-overlay-context` - Bind VID overlay edit context (F4 / overlay editor); renamed from `set-overlay-context`
...
**Note:** When saving as a `.red` file, a companion `.redbin` file is also created with the same filename. The `.redbin` file contains all configuration, overlay and customization data that is contained within the table. The original `.red` format only saved the data and some small number of configuration options. This dual-file approach is provided as a transition format to allow existing 'old' `.red` files to still be loaded and saved while preserving all table customizations. Loading that companion `.redbin` data is skipped safely when `face/data` is missing, not a file, or the expected redbin path does not exist, so incomplete paths do not error at load time.
